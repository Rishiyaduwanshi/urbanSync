...
module.exports = {
  ...
  deleteProjectManager: async (req, res) => {
    try {
      const email = parseInt(req.params.email); // Extract and parse email from params
      const result = await ProjectManager.findOneAndDelete({ email }); // Use findOneAndDelete if email is a number
      if (!result) {
        return res
          .status(404)
          .json({
  ...
  updateProjectManager: async (req, res) => {
    try {
      ...
      const result = await ProjectManager.findOneAndUpdate(
        { email },
        updatedData,
        { new: true }
      );
  ...
  getProjectManagers: async (req, res) => {
    try {
      ...
      res.status(200).json({ status: "Success", data: allProjectManagers });
    } catch (err) {
      res
        .status(500)
        .json({
