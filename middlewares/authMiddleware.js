...
const jwt = require('jsonwebtoken');
const User = require('../models/Administrator.model'); // Adjusted to match the User model if needed
...
const authorizeAdmin = async (req, res, next) => {
  try {
    const user = await User.findById(req.user.userId); // Changed to match userId in token payload
